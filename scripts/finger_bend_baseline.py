...
WINDOW_NAME = "Finger Bend Setup"
TRACK_WINDOW_NAME = "Finger Bend Tracking"
POINT_COUNT = 3
...
def parse_args() -> argparse.Namespace:
    ...
    default_video = repo_root / "data" / "IMG_5935.mov"
    default_output = repo_root / "analysis" / "finger_bend"

    parser = argparse.ArgumentParser(
        description=(
            "Baseline finger-bend tracker using OpenCV optical flow. "
            "Click 3 points along the finger in proximal-to-distal order. "
    ...
    parser.add_argument(
        ...
        help="Path to the input video. Defaults to data/IMG_5935.mov.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def annotate_frame(
    ...
    cv2.putText(
        annotated,
        f"Flexion: {flexion_degrees:6.2f} deg",
        (20, 105),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.8,
        (255, 255, 255),
    ...
    cv2.putText(
        annotated,
        "Straight finger is close to 0 deg flexion.",
        (20, 140),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (255, 255, 255),
...
def select_initial_points(frame: np.ndarray) -> np.ndarray:
    ...
    def redraw() -> None:
        ...
        display_frame = frame.copy()
        instructions = [
            "Click 3 points along one finger in order:",
...
def process_video(
    ...
    annotated_video_path = output_dir / "finger_bend_annotated.mp4"
    ...
    csv_path = output_dir / "finger_bend_measurements.csv"
